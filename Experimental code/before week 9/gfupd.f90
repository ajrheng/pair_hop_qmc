...
!=====================!
 ...
 !integer :: i,j,ii,o,b,s1,s2,jjj,sa,su
 !integer :: nc,is,js,ks,cnl(nn,nn),cst(nn,nn),strphs
 !integer :: ss1,ss2,tt1,tt2,nu,nk,nh1,last,ph1,ph2
 !real(8) :: ssum,sstr,ssus,dl
 !real(8) :: rndm,p,addp,delp
 ...
 !do i=1,l
    !ii=gstring(i)
    !if (ii.eq.0) then
       !b=min(int(rndm()*dble(nb))+1,nb)
        !ss1=st(bst(1,b)); ss2=st(bst(2,b))
    ...
    !else
       !b=ii/3
       !o=mod(ii,3)
       !s1=bst(1,b); s2=bst(2,b)
       !ss1=st(s1); ss2=st(s2)
       !ph1=phase(s1); ph2=phase(s2)
       !nk=nk+1
       !nh1=nh1+1
       !dl=dble(nh1-last)
       !ssum=ssum+dl*dble(sa)
       !sstr=sstr+dl*dble(sa)**2
       !st(s1)=op1(o,ss1,ss2); st(s2)=op2(o,ss1,ss2)
       !tt1=st(s1); tt2=st(s2)
       !sa=sa+ph1*(tt1-ss1)+ph2*(tt2-ss2)
       !jjj=jjj+stra(o,ss1,ss2)
       !last=nh1
    !endif
 !enddo
 !dl=dble(nh1-last)
 ...
 !ssa=ssa+sstr
 !sxa=sxa+beta*ssus
 !rho=rho+(dble(jjj)**2)/(dble(nn)*beta)

!end subroutine gdupdate
!=====================!
