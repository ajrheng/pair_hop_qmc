...
real(8) :: vxprb(0:7,0:7,nvx),corr(0:nx/2,0:ny/2),strFactTemp(0:nx,0:nx),strFact(0:nx,0:nx)
  ...
  do k2=0,nx
    do dx=0,nx/2
      do dy=0,ny/2
        ...
        cosk1=DCOS(dble(k1)*2*pi*dble(dx)/nx)
        !sink1=DSIN((dble(k1)/50)*pi*dx)
        sink1=DSIN(dble(k1)*2*pi*dble(dx)/nx)
        !cosk2=DCOS((dble(k2)/50)*pi*dy)
        cosk2=DCOS(dble(k2)*2*pi*dble(dy)/nx)
        !sink2=DSIN((dble(k2)/50)*pi*dy)
        sink2=DSIN(dble(k2)*2*pi*dble(dy)/nx)
        strFactTemp(k1,k2)=strFactTemp(k1,k2)+((cosk1*cosk2-sink1*sink2)*corr(dx,dy))
      enddo
    enddo
  enddo
enddo
