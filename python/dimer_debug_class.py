...
class mc_sse_dimer:
    ...
    def vxweight(self):
        ...
        for iq in range(self.MAX_BOND_NUM):
            if self.awgt[iq] != 0:
                ...
                #print('nvx: {0}, ns0: {1}, ns1: {2}, ns2: {3}, ns3: {4}'.format(self.nvx,ns[0],ns[1],ns[2],ns[3]))
                        ...
                        if (self.act_tp[outstate]!=-1):
                            ...
                            if (self.vx_num_from_int[iiv]!=-1):
                                ...
                                if tup not in debug:
                                    ...
                                    print('tup {0} is already inside!!'.format(tup))
    ...
    def diagonal_update(self,track_step,track_run):
        ...
        for i in range(self.l):
            ii = self.opstring[i]
            if ii == 0:
                b = random.randint(1,self.nb) #randint is inclusive of end
        ...
        for _ in range(self.t_loop_len):
            ...
            for i in range(ml):
                ...
                for out_leg in range(4):
                    ...
                    if out_leg == 3:
                        with open('error.txt','a') as file:
                            file.write("didn't find a suitable outleg for t worm on  run {0} step {1}!\n".format(track_run,track_step)) #should have reached break statement previously
        ...
        for _ in range(self.d_loop_len):
            ...
            for i in range(ml):
                ...
                for out_leg in range(4):
                    ...
                    if out_leg == 3:
                        with open('error.txt','a') as file:
                            file.write("didn't find a suitable outleg for d worm on run {0} step {1}!\n".format(track_run, track_step)) #should have reached break statement previously
        ...
        for i in range(self.nn):
            if self.first[i] != -1:
                ...
                if self.state[i] not in [0,1,2,3]:
                    with open('error.txt','a') as file:
                        file.write("wrong state on run {0} step {1}!\n".format(track_run, track_step))
