...
class getItems():
	...
	def get_dat(self,dattype):
		...
		for x in items:
			...
			# it starts with Vd and construct a new list to put them into.
			if x[:2] == "Vd":
				m=float(x[3:])
		...
		for each in items_removed:
			# /Users/Alvin/Desktop/cy2001 results/varyVdwhenv=5/Vd=30/uni.dat
			#each = subfolder.
			#{ subfolder | mean |  sd }
	...
	def get_csv_headers(self,dattype):
		if dattype == "uni":
			csv_List = [["Vd","Mean","STDEV"]]
		elif dattype == "rho":
			csv_List = [["Vd","rho mean","STDEV"]]
		elif dattype == "rhot":
			csv_List = [["Vd","rhot mean","STDEV"]]
		elif dattype =="rhotp":
			csv_List = [["Vd","rhotp mean","STDEV"]]
		elif dattype == "stgpipi":
			csv_List = [["Vd","ssa mean","STDEV"]]
